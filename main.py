...
PEOPLE = [
    {
        ...
        "birthday_message": """🎉🎂 تولدت مبارک خوشگل خانم 💖
...
"""
    ...
    if days > 100:
        text = header + random.choice(normal_messages).format(days=days)

    elif days == 100:
        text = header + f"💯 فقط 100 روز تا تولد {name} مونده 🎉"

    elif days > 50:
        text = header + random.choice(normal_messages).format(days=days)

    elif days == 50:
        text = header + f"🌸 فقط 50 روز مونده تا روز قشنگ {name} 💖"

    elif days > 30:
        text = header + random.choice(normal_messages).format(days=days)

    elif days == 30:
        text = header + f"""✨ فقط 30 روز مونده
...
کم‌کم داریم به روز قشنگ {name} نزدیک می‌شیم 🌷"""
        ...
        text = header + f"💖 فقط دو هفته تا تولد {name} مونده ✨"

    elif days > 7:
        text = header + random.choice(normal_messages).format(days=days)

    elif days == 7:
        text = header + f"🎈 فقط یک هفته تا تولد {name} مونده ❤️"

    elif days > 3:
        text = header + random.choice(normal_messages).format(days=days)

    elif days == 3:
        text = header + f"🎁 فقط 3 روز دیگه تا تولد {name} مونده 🥳"

    elif days == 2:
        text = header + f"💕 فقط 2 روز تا تولد {name} مونده 🤍"

    elif days == 1:
        text = header + f"""🌙 فردا تولد {name} است...

امیدوارم فردا یکی از قشنگ‌ترین روزهای زندگیت باشه 🤍✨"""

    elif days == 0:
        text = header + person["birthday_message"]

    else:
        text = header + f"🎂 تولد {name} امسال گذشت، حالا منتظر سال بعد می‌مونیم ❤️"
